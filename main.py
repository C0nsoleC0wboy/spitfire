...
if __name__ == '__main__':
    ...
    try:
        ac = live_ac.search_radius(51.30457774177224, -0.09535818745250407, 7)
    except caa.client.RequestException:
        ac = []

    ac_dict = dict(map(lambda x: (x[0], (x[1], x[2])), (filter(lambda x: x if x[0] else None, ac))))
    ac_set = set(map(lambda x: x[0][2:], filter(lambda x: x if x[0] and x[0][:2] == 'G-' else None, ac)))

    alert_ac = ac_set & caa.war_ac

    for p in alert_ac:
        if p not in prev_ac:
            plane_details = ac_dict[f'G-{p}']
